// Levels.js - Level selection page with 24 levels, lock system, admin bypass, and coming soon
...
const Levels = ({ user }) => {
  ...
  // Check if user is admin - unlocks all 24 levels
  useEffect(() => {
  ...
  // Gradients for levels (4 levels per group for 24 levels)
  const getGradientForLevel = (level) => {
    const group = Math.ceil(level / 4);
    const variation = level % 4;
